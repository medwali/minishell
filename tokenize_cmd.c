...
/*   Updated: 2020/11/15 00:07:52 by mel-idri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector	*tokenize_cmd(char *cmd)
{
	...
	vector_push(tokens, &(t_token){CMD_START, NULL});
	while (*cmd)
	{
		if (*cmd == '"')
		...
		{
			cmd = skip_chars(cmd, " \t");
			vector_push(tokens, &(t_token){SEPARATOR, NULL});
		}
		else if (*cmd == '~' && is_tilde(cmd, tokens))
		{
			cmd++;
			vector_push(tokens, &(t_token){TILDE, NULL});
		}
	...
	vector_push(tokens, &(t_token){CMD_END, NULL});
	return (tokens);
}
