...
/*   Updated: 2020/11/15 00:08:58 by mel-idri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	tokenize_double_quotes(char **cmd, t_vector *tokens)
{
	...
	vector_push(tokens, &(t_token){DBL_QT_START, NULL});
	(*cmd)++;
	while (1)
	{
		start = *cmd;
		...
		if (*cmd != end)
		{
			...
			vector_push(tokens, &(t_token){STRING, content});
		}
		*cmd = end;
		if (**cmd == '$')
	...
	vector_push(tokens, &(t_token){DBL_QT_END, NULL});
	(*cmd)++;
}
